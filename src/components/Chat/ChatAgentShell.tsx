...
export default function ChatAgentShell({
  ...
  return (
    <div
      ...
      className={`flex h-full min-h-0 w-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-[#FBFCFD] shadow-[0_10px_40px_-18px_rgba(15,23,42,0.28)] ${className}`.trim()}
      style={{
        ...themeStyle,
        borderTopWidth: 3,
    ...
    >
      <header
        data-chat-header
        className="flex shrink-0 items-center gap-2.5 border-b border-slate-200 bg-white px-3.5 py-2.5"
      >
        <div className="relative shrink-0">
          {showImage ? (
            <img
              ...
              className="h-9 w-9 rounded-full bg-white object-contain p-0.5 ring-1 ring-slate-200"
            />
          ) : (
            <div
              data-compact-avatar-fallback
              className="flex h-9 w-9 items-center justify-center rounded-full text-sm font-semibold ring-1 ring-slate-200"
              style={{
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-[14px] font-semibold leading-5 tracking-tight text-slate-900">
            {name}
          </h2>
